...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>產品接入協議與認證配置</strong>：每台設備單獨約定協議與鑒權，是聯調返工的重災區。平臺在產品級一次定稿接入協議（MQTT / TCP / HTTP）、數據格式、認證方式與加解密策略，下屬設備自動繼承同一套規範——聯調時不再逐台約定鑒權與報文格式，接入規範從「人口口相傳」變成「產品級可繼承契約」</li>
