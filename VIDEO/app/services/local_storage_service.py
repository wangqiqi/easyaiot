...
def get_local_storage_root() -> str:
    """flat 本地对象根目录（edge/mini 均走本地盘，不依赖 MinIO 服务）。"""
    ...
    # Docker 媒体卷固定挂载点（容器内路径；宿主机 EASYAIOT_MEDIA_ROOT 不可直接当路径用）
    if os.path.isdir('/mnt/easyaiot-media'):
        return '/mnt/easyaiot-media/local-storage'
    for key in ('MEDIA_HOST_DATA_ROOT', 'SRS_HOST_DATA_ROOT', 'EASYAIOT_MEDIA_ROOT'):
...
def _video_root() -> str:
    video_root = (os.getenv('VIDEO_ROOT') or os.getenv('EASYAIOT_VIDEO_ROOT') or '').strip()
    if video_root:
        return os.path.normpath(video_root)
    return os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))


def get_minio_seed_data_root() -> Optional[str]:
    """安装包 flat 种子目录（.scripts/minio）。

    仅目录结构沿用历史 bucket/key 布局，edge 不启 MinIO；权重/封面复制到 local-storage。
    """
    for key in (
        'MODEL_SEED_DATA_ROOT',
        'MINIO_SEED_DATA_ROOT',
        'MINIO_FS_DATA_ROOT',
    ):
    ...
    repo_root = os.path.dirname(_video_root())
    candidates = [
        '/model-seed-data',
        '/minio-seed-data',
