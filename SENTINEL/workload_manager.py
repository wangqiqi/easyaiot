...
class WorkloadManager:
    ...
    def deploy(self, spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        runtime_lib = '/opt/easyaiot/RUNTIME/lib'
        if os.path.isdir(runtime_lib):
            current_ld = env.get('LD_LIBRARY_PATH', '')
            parts = [p for p in current_ld.split(':') if p]
            if runtime_lib not in parts:
                env['LD_LIBRARY_PATH'] = ':'.join([runtime_lib] + parts)
        if gpu_ids:
...
def _ensure_model_local(model_path: str, model_id: str) -> Optional[str]:
    """若 MODEL_PATH 为 MinIO URL 则下载到本地；集群模式优先读 NFS 共享缓存。"""
    try:
        mid = int(model_id)
        if mid > 0:
