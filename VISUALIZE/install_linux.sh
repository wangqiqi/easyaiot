...
# ============================================
# VISUALIZE 可视化编辑器 Docker Compose 管理脚本
# ============================================
# 使用方法：
#   ./install_all.sh [命令]
#
...
# 远程镜像公共库（aiot-visualize-web 拉取与 runtime_image.sh 共用命名规则）
if [ -z "${RUNTIME_IMAGE_COMMON_LOADED:-}" ]; then
    _visualize_saved_script_dir="$SCRIPT_DIR"
    ...
    SCRIPT_DIR="$_visualize_saved_script_dir"
fi
...
# 清理 compose recreate 被中断后遗留的「改名孤儿容器」（形如 <12位hex>_visualize-service）。
...
cleanup_renamed_containers() {
    local names
    names=$(docker ps -a --format '{{.Names}}' 2>/dev/null | grep -E '^[0-9a-f]{12}_visualize-service$' || true)
    [ -z "$names" ] && return 0
...
# 拉取远程预构建 VISUALIZE 镜像（aiot-visualize-web:<arch> → visualize-service:latest，与 runtime_image.sh 一致）
visualize_pull_prebuilt_image() {
    runtime_load_registry
    export REGISTRY="${REGISTRY:-$RUNTIME_IMAGE_REGISTRY}"
    local arch; arch=$(runtime_detect_arch)
    local rref; rref=$(runtime_remote_ref "aiot-visualize-web" "" "$arch")
    local lref; lref=$(runtime_local_ref "visualize-service")
...
visualize_skip_build_from_pull() {
    [ "${EASYAIOT_SKIP_BUILD:-0}" != "1" ] && return 1
    docker image inspect visualize-service:latest >/dev/null 2>&1 || return 1
    print_success "镜像已从远程拉取 (visualize-service:latest)，跳过 Docker 构建与 vite 编译"
    return 0
}

# 组合 git 提交与 clean 写入的戳，用于 Dockerfile ARG CACHE_BUST（使 COPY 之后层在代码/clean 后重建）
get_visualize_build_cache_bust() {
    local git_rev stamp
    git_rev=$(git -C "$SCRIPT_DIR" rev-parse HEAD 2>/dev/null || echo "nogit")
    stamp=""
    if [ -f "$(visualize_build_stamp_file "$EASYAIOT_ROOT")" ]; then
        stamp=$(tr -d '\n' < "$(visualize_build_stamp_file "$EASYAIOT_ROOT")")
    fi
    echo "${git_rev}-${stamp}"
}

# 执行 docker build（BuildKit + 宿主机 .build-cache/visualize/pnpm-store 本地 cache 后端）
docker_build_image() {
    ...
    cache_bust=$(get_visualize_build_cache_bust)
    {
        echo ""
    ...
    set -o pipefail
    local pnpm_store
    pnpm_store="$(visualize_pnpm_store_dir "$EASYAIOT_ROOT")"
    mkdir -p "${pnpm_store}"
    # 构建层缓存策略：
    ...
    # 易失/CI 环境（构建后守护进程缓存会被清理、或换机器）可设 VISUALIZE_PERSIST_BUILD_CACHE=1 重新启用持久化。
    local cache_from_to=()
    if [ "${VISUALIZE_PERSIST_BUILD_CACHE:-0}" = "1" ]; then
        ...
        print_info "持久化构建缓存到 ${pnpm_store}（VISUALIZE_PERSIST_BUILD_CACHE=1，会增加每次构建的导出耗时）"
    else
        print_info "使用 Docker 守护进程层缓存（默认）；如需跨守护进程清理/换机器持久化：VISUALIZE_PERSIST_BUILD_CACHE=1"
    fi
    local platform_opts=""
...
handle_port_conflict() {
    ...
    if [ ${#pids[@]} -eq 0 ]; then
        ...
        print_info "请手动停止占用端口的进程，或修改 .env 文件中的 VISUALIZE_PORT 配置"
        return 1
    fi
    ...
    case "${choice:-2}" in
        1)
            ...
            if [ $failed_count -gt 0 ]; then
                ...
                print_info "请手动处理或修改 .env 文件中的 VISUALIZE_PORT 配置"
                return 1
            else
...
check_port() {
    local port=$1
    if [ -z "$port" ]; then
        # 从.env文件读取端口，如果没有则使用默认值8002
        if [ -f .env ]; then
            port=$(grep "^VISUALIZE_PORT=" .env 2>/dev/null | cut -d '=' -f2 | tr -d ' ' | tr -d '"' | tr -d "'")
        fi
        port=${port:-8002}
    fi
...
verify_upstream_connectivity() {
    local container_name="visualize-service"
    # 等待容器完全启动
    local attempt=0
    ...
    # 检查 gateway 可达性（VISUALIZE 仅 full，经 iot-gateway:48080）
    ...
    if docker exec "$container_name" wget -q --timeout=3 --tries=1 --spider "http://${upstream_host}:${upstream_port}/" 2>/dev/null; then
        ...
        print_info "待后端就绪后重启 VISUALIZE 服务: ./install_linux.sh restart"
    fi
}

# 安装服务
install_service() {
    print_info "开始安装 VISUALIZE 服务..."

    ensure_deploy_profile
    if [ "${EASYAIOT_DEPLOY_PROFILE:-full}" != "full" ]; then
        print_warning "VISUALIZE 可视化编辑器 仅在 full 全量部署形态下启用，当前: ${EASYAIOT_DEPLOY_PROFILE}，跳过"
        return 0
    fi
    ...
    if [ "${EASYAIOT_SKIP_IMAGE_PROMPT:-0}" = "1" ]; then
        if [ "${EASYAIOT_DEPLOY_PROFILE:-full}" = "full" ] \
            && ! docker image inspect visualize-service:latest >/dev/null 2>&1; then
            print_info "本地无 visualize-service 镜像，尝试从远程仓库拉取 aiot-visualize-web..."
            visualize_pull_prebuilt_image && export EASYAIOT_SKIP_BUILD=1 \
                || print_warning "VISUALIZE 预构建镜像拉取失败，将尝试本地构建"
        fi
    else
        ...
        if [ "$_do_local_build" -eq 0 ]; then
            ...
            if visualize_pull_prebuilt_image; then
    ...
    docker rm -f visualize-service 2>/dev/null || true
    ...
    if visualize_skip_build_from_pull; then
        :
    elif [ "${EASYAIOT_SKIP_BUILD:-0}" = "1" ] && docker image inspect visualize-service:latest >/dev/null 2>&1; then
        print_success "镜像已从远程拉取 (visualize-service:latest)，跳过 Docker 构建与 vite 编译"
    else
        print_info "H5 前端构建将在 Docker 容器内自动完成"
        print_info "构建 Docker 镜像（根据代码重新构建）..."
        docker_build_image -t visualize-service:latest .
    fi
    ...
    # 读取端口配置
    if [ -f .env ]; then
        VISUALIZE_PORT=$(grep "^VISUALIZE_PORT=" .env 2>/dev/null | cut -d '=' -f2 | tr -d ' ' | tr -d '"' | tr -d "'")
    fi
    VISUALIZE_PORT=${VISUALIZE_PORT:-8002}
    print_info "服务访问地址: http://localhost:${VISUALIZE_PORT}"
    print_info "健康检查地址: http://localhost:${VISUALIZE_PORT}/health"
...
check_status() {
    ...
    echo ""
    print_info "容器健康状态:"
    if docker ps --filter "name=visualize-service" --format "table {{.Names}}\t{{.Status}}" | grep -q visualize-service; then
        docker ps --filter "name=visualize-service" --format "table {{.Names}}\t{{.Status}}"
        
        # 检查健康检查
        HEALTH=$(docker inspect --format='{{.State.Health.Status}}' visualize-service 2>/dev/null || echo "N/A")
        if [ "$HEALTH" != "N/A" ]; then
            echo "健康状态: $HEALTH"
        fi
    else
...
build_image() {
    ...
    local build_rc=0
    docker_build_image -t visualize-service:latest --no-cache . || build_rc=$?
...
clean_service() {
    ...
    print_info "停止并删除容器..."
        ...
        print_info "删除镜像..."
        docker rmi visualize-service:latest 2>/dev/null || true
        ...
        init_easyaiot_build_cache_dirs "$EASYAIOT_ROOT"
        date +%s > "$(visualize_build_stamp_file "$EASYAIOT_ROOT")"
        print_info "已更新 .build-cache/visualize/.build-stamp，下次构建将重新编译 VISUALIZE 前端（.build-cache/visualize/pnpm-store 依赖缓存保留）"
...
#   3. 需要构建时仍复用 .build-cache/visualize/pnpm-store 依赖缓存（docker_build_image 已内置），
...
update_service() {
    ...
    # 说明3：部署形态变更时 VISUALIZE 模块本身不可用（仅 full），此处不校验形态戳
    if [ "${FORCE_REBUILD:-0}" != "1" ] \
        && docker image inspect visualize-service:latest >/dev/null 2>&1 \
...
# 显示帮助信息
show_help() {
    echo "VISUALIZE 可视化编辑器 Docker Compose 管理脚本"
    echo ""
    echo "使用方法:"
