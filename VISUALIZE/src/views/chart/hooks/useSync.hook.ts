import { getUUID, fetchRouteParamsLocation, JSONParse, JSONStringify } from '@/utils'
...
import { fetchProjectApi, saveProjectContentApi } from '@/api/path'
...
export const useSync = () => {
  ...
  const dataSyncFetch = async () => {
    ...
    try {
      ...
      if (!project) {
        window['$message']?.warning('项目不存在或无权访问')
        return
      }
      if (project.projectName) {
        document.title = `${project.projectName} - EasyAIoT`
      }
      if (project.content) {
        const content =
          typeof project.content === 'string' ? JSONParse(project.content) : project.content
        await updateComponent(content, true)
      }
    } catch (e) {
      // 拦截器已提示
    } finally {
  ...
  /** 保存画布到后台 */
  const dataSyncUpdate = async (updateInterval?: boolean) => {
    const id = fetchRouteParamsLocation()
    if (!id) return
    try {
      const content = JSONStringify(chartEditStore.getStorageInfo())
      await saveProjectContentApi({ id, content })
      if (!updateInterval) {
