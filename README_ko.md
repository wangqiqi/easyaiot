...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>제품 접속 프로토콜·인증 구성</strong>: 기기마다 프로토콜·인증을 따로 정하는 것은 연동 재작업의 온상입니다. 제품 수준에서 접속 프로토콜(MQTT / TCP / HTTP)·데이터 형식·인증 방식·암복호화 전략을 한 번에 확정하고 하위 디바이스가 동일 규범을 자동 상속 — 연동 시 기기마다 인증·메시지 형식을 다시 약속하지 않음; 접속 규범이 「입소문」에서 「제품급 상속 계약」으로</li>
