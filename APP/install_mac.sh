...
# ============================================
# APP 移动端 H5 Docker Compose 管理脚本 (macOS 版本)
# ============================================
# 使用方法：
#   ./install_mac.sh [命令]
#
...
install_service() {
    ...
    if [ "${EASYAIOT_DEPLOY_PROFILE:-full}" != "full" ]; then
        print_warning "APP 移动端 H5 仅在 full 全量部署形态下启用"
        exit 1
    fi
    ...
    if [ "${EASYAIOT_SKIP_BUILD:-0}" = "1" ] && docker image inspect app-service:latest >/dev/null 2>&1; then
        print_success "镜像已从远程拉取 (app-service:latest)，跳过构建"
    else
...
build_image() {
    ...
    $COMPOSE_CMD build --no-cache 2>/dev/null || docker build -t app-service:latest --no-cache .
...
# 显示帮助信息
show_help() {
    echo "APP 移动端 H5 Docker Compose 管理脚本 (macOS 版本)"
    echo ""
    echo "使用方法:"
