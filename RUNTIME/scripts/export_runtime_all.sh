...
#
# 用法:
#   bash RUNTIME/scripts/export_runtime_all.sh              # 默认实验室优先包
#   bash RUNTIME/scripts/export_runtime_all.sh --all        # 全矩阵
#   bash RUNTIME/scripts/export_runtime_all.sh --compile-target openeuler
#   bash RUNTIME/scripts/export_runtime_all.sh openeuler22 el9 ubuntu26
#   RUNTIME_MATRIX_JOBS=2 bash RUNTIME/scripts/export_runtime_all.sh --all
# ============================================
set -euo pipefail
...
usage() {
  cat <<EOF
用法: $0 [--all|--default|--compile-target <name>] [os_family ...]

  --default              仅打 RUNTIME_MATRIX_DEFAULT（默认）
  --all                  打 runtime_os_matrix.sh 登记的全部目标
  --compile-target NAME  与 COMPILE/build.sh 同名目标，如 centos-el9 / openeuler / ubuntu-x86
  os_family ...          显式列表，如 openeuler22 el9 ubuntu26

环境变量:
  RUNTIME_MATRIX_JOBS=1  并行容器数（Miniconda 按 os 分卷，可安全并行）
  RUNTIME_MATRIX_DEFAULT 覆盖默认目标列表
EOF
}
...
resolve_keys() {
  ...
  if [[ ${#TARGETS[@]} -gt 0 ]]; then
    ...
    keys="$(runtime_matrix_default_keys)"
  fi
  echo "$keys"
}
