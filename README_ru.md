...
<ul style="font-size: 14px; line-height: 1.8; color: #444; margin: 10px 0;">
  ...
  <li><strong>Протокол доступа и аутентификация на уровне продукта</strong>: Отдельные договорённости о протоколе и auth на каждое устройство — очаг переделок интеграции. На уровне продукта один раз фиксируются протокол (MQTT / TCP / HTTP), формат данных, способ аутентификации и стратегия шифрования/дешифрования; подчинённые устройства наследуют тот же контракт — больше не согласовывать auth и формат кадр за кадром; норма доступа из «устной передачи» становится «наследуемым контрактом уровня продукта»</li>
