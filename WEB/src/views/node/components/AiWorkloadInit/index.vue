<template>
  <div class="ai-workload-init">
    ...
    >
      <TabPane v-for="bundle in AI_WORKLOAD_BUNDLE_TYPES" :key="bundle.key" :tab="bundle.label" />
    </Tabs>

    <div class="bundle-tab-content">
      <template v-for="bundle in AI_WORKLOAD_BUNDLE_TYPES" :key="bundle.key">
        <WorkloadBundlePanel
...
import { AI_WORKLOAD_BUNDLE_TYPES } from '../../utils/constants';
...
defineOptions({ name: 'AiWorkloadInit' });
...
const defaultBundle =
  AI_WORKLOAD_BUNDLE_TYPES.find((b) => b.key === bundleFromQuery)?.key ||
  AI_WORKLOAD_BUNDLE_TYPES[0]?.key ||
  'ai_service';
