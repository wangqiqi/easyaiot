...
export const unitOptions = [
  {
    ...
    sort: 1,
    status: 1,
    sysDict: { dictName: '参数单位', dictCode: 'attribute_unit' },
  },
  {
...
export const getBasicColumns = (functionType : String): BasicColumn[] => {
  if(functionType == 'services') {
    return [
      ...
      {
        title: '是否标准功能',
        dataIndex: 'templateIdentification',
        customRender({ value }) {
          return <Tag color={value !== null ? 'green' : 'red'}>{value !== null ? '是' : '否'}</Tag>;
        },
      },
    ];
      ...
      },
      {
        ...
        customRender({ text }) {
          if (text === 'INFO_EVENT_TYPE') {
            return <Tag color='green'>信息</Tag>;
          } else if (text === 'ALERT_EVENT_TYPE') {
            return <Tag color='red'>告警</Tag>;
          } else if (text === 'ERROR_EVENT_TYPE') {
            return <Tag color='yellow'>故障</Tag>;
          }else{
            return text ?? '--';
          }
        },
...
export const PropsSchemas = ({ handleChange, isInner }: SchemasFn): FormSchema[] => {
  return [
    ...
    },
    {
      ...
      label: '参数标识',
      component: 'Input',
      rules: [{ required: true, message: '请输入编码', trigger: 'change' }],
      colProps: {
        span: 24,
      },
    },
