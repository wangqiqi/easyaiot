package com.basiclab.iot.device.domain.ota.oo;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;

import javax.validation.constraints.NotBlank;
import javax.validation.constraints.NotNull;
import java.io.Serializable;

/**
 * 设备升级结果上报（统一出入口：四类包共用一个上报接口）
 *
 * @author 翱翔的雄库鲁
 * @email andywebjava@163.com
 * @wechat EasyAIoT2025
 */
@Data
@ApiModel(value = "DmOtaReportOo对象", description = "设备升级结果上报Oo")
public class DmOtaReportOo implements Serializable {

    private static final long serialVersionUID = 1L;

    /**
     * 设备唯一标识
     */
    @NotBlank(message = "设备唯一标识不能为空")
    @ApiModelProperty(value = "设备唯一标识")
    private String deviceIdentification;

    /**
     * 产品标识（可选）
     */
    @ApiModelProperty(value = "产品标识（可选）")
    private String productIdentification;

    /**
     * 包类型[0:软件包,1:固件包,2:APP包,3:PC包]
     */
    @NotNull(message = "包类型不能为空")
    @ApiModelProperty(value = "包类型[0:软件包,1:固件包,2:APP包,3:PC包]")
    private Integer type;

    /**
     * 升级前版本
     */
    @ApiModelProperty(value = "升级前版本")
    private String fromVersion;

    /**
     * 升级目标版本
     */
    @NotBlank(message = "升级目标版本不能为空")
    @ApiModelProperty(value = "升级目标版本")
    private String toVersion;

    /**
     * 通道[1:测试,2:正式]（不传由服务端按检测结果判定）
     */
    @ApiModelProperty(value = "通道[1:测试,2:正式]")
    private Integer channel;

    /**
     * 升级阶段[0:检测,1:命中,2:下载完成,3:下载失败,4:MD5校验失败,5:安装结果,6:启动成功]
     */
    @NotNull(message = "升级阶段不能为空")
    @ApiModelProperty(value = "升级阶段[0:检测,1:命中,2:下载完成,3:下载失败,4:MD5校验失败,5:安装结果,6:启动成功]")
    private Integer phase;

    /**
     * 升级进度（0-100）
     */
    @ApiModelProperty(value = "升级进度（0-100）")
    private Integer progress;

    /**
     * 是否成功[0:否,1:是]
     */
    @ApiModelProperty(value = "是否成功[0:否,1:是]")
    private Integer success;

    /**
     * 错误码
     */
    @ApiModelProperty(value = "错误码")
    private String errorCode;

    /**
     * 错误信息
     */
    @ApiModelProperty(value = "错误信息")
    private String errorMsg;

    /**
     * 升级耗时（毫秒）
     */
    @ApiModelProperty(value = "升级耗时（毫秒）")
    private Long costMs;
}
